...
class ComponentMeta(type):
    ...
    def __new__(cls, name, bases, d):
        ...
        # if the class directly or indirectly implements any ExtensionInterfaces, 
        # register the class as a provider of that extension
        for interface in _pre_advice_all_interfaces(new_class):
            if interface.isOrExtends(ExtensionInterface):
                registry.setdefault(interface, [])
                if not new_class in registry[interface]:
                    registry[interface].append(new_class)

        return new_class

def _pre_advice_all_interfaces(cls):
    advice = cls.__dict__.get('__implements_advice_data__', None)
    if advice != None:
        interfaces, classImplements = advice
        all_ifaces = set()
        for iface in interfaces:
            all_ifaces.add(iface)
            all_ifaces.update(iface._implied)
        return all_ifaces
    else:
        return []
